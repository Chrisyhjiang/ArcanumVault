...
def rotate_key_periodically():
    ...
    old_cipher = cipher
    new_cipher = Fernet(new_key)
    print("Re-encrypting all passwords with the new key (periodic task)...")
    reencrypt_passwords(old_cipher, new_cipher)
    store_new_key(new_key)
    print("Periodic key rotation completed successfully.")
    logging.info("Periodic key rotation completed successfully")  # Log successful rotation

# Background task function
def run_periodic_task():
    while True:
        time.sleep(1800)  # 30 minutes interval
        logging.info("Starting periodic key rotation")  # Log before rotation
        rotate_key_periodically()
        logging.info("Completed periodic key rotation")  # Log after rotation
