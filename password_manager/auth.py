...
from .constants import SESSION_FILE, SESSION_TIMEOUT, MASTER_PASSWORD_FILE
from .utils import load_secure_key, set_permissions, decrypt_master_password
...
def authenticate_fingerprint_mac():
    ...
    if success:
        ...
        semaphore = CDLL('/usr/lib/system/libdispatch.dylib').dispatch_semaphore_create(0)
        ...
        def callback(_success, _error):
            ...
            if _success:
                ...
                authenticated = False
            CDLL('/usr/lib/system/libdispatch.dylib').dispatch_semaphore_signal(semaphore)
        ...
        CDLL('/usr/lib/system/libdispatch.dylib').dispatch_semaphore_wait(semaphore, c_long(-1))
